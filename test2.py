    ...
    for episode in range(num_episodes):
        ...
        while state != GOAL:  # 训练一次
            ...
            if sample_num > eps_threshold:  # 利用
                ...
                for j in next_state_ac_available:
                    next_state_dict[j] = q_value[next_state, j]
                    action = [key_ for key_, value_ in next_state_dict.items() if value_ == max(list(next_state_dict.values()))]

            else:  # 探索
                action = random.sample(next_state_ac_available, 1)

            next_next_state, reward = step(next_state, action[0])

            loss2 = next_next_state - next_state
            if (loss1 + loss2) == 0:
                reward = -1000

            next_next_state_ac_available = []
            node_next_next_state = node[next_next_state]

            for i in node_next_next_state.keys():
                next_next_state_ac_available.append(i)

            next_next_state_q_value = [q_value[next_next_state, j] for j in next_next_state_ac_available]
            next_next_state_max_q_value = max(next_next_state_q_value)

            # Q-learning algorithm !!
            q_value[next_state, action[0]] += ALPHA * (reward + gamma * next_next_state_max_q_value - q_value[next_state,action[0]])
